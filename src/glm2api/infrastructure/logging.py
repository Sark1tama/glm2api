...
_SENSITIVE_DEBUG_TITLE_PATTERN = re.compile(
    r"(?i)(authorization\s*[:=]\s*bearer\s+)[^\s,&]+|"
    r"((?:access[_-]?token|refresh[_-]?token|api[_-]?key|token|signature|sig)\s*[:=]\s*)[^\s,&]+"
)
...
def _redact_debug_value(value: Any) -> Any:
    """Redact credentials from structured debug payloads before logging."""
    if isinstance(value, bytes):
        try:
            ...
            return value
        return _redact_debug_value(parsed)
    if isinstance(value, str):
        stripped = value.strip()
        if stripped.startswith(("{", "[")):
            try:
                return _redact_debug_value(json.loads(stripped))
    ...
    if isinstance(value, Mapping):
        ...
        for key, item in value.items():
            if isinstance(key, str) and key.strip().lower() in _SENSITIVE_DEBUG_KEYS:
...
def _redact_debug_title(title: str) -> str:
    return _SENSITIVE_DEBUG_TITLE_PATTERN.sub(
...
def debug_dump(logger: logging.Logger, enabled: bool, title: str, value: Any) -> None:
    if not enabled:
        return
    logger.debug("%s\n%s", _redact_debug_title(title), serialize_for_debug(_redact_debug_value(value)))
