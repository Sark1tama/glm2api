...
DEFAULT_CHAT_MODEL_NAME = "glm-5.3-flash"
DEFAULT_IMAGE_MODEL_NAME = "glm-image-1"
DEFAULT_VIDEO_MODEL_NAME = "glm-video-1"
DEFAULT_GLM_BASE_URL = "https://chatglm.cn/chatglm"
DEFAULT_MAX_REQUEST_BODY_BYTES = 101 * 1024 * 1024
...
BUILTIN_MODEL_CATALOG = {
    "glm-5.3": ModelSpec(kind="text", multimodal=False),
    DEFAULT_CHAT_MODEL_NAME: ModelSpec(kind="text", multimodal=True),
    DEFAULT_IMAGE_MODEL_NAME: ModelSpec(kind="image"),
    DEFAULT_VIDEO_MODEL_NAME: ModelSpec(kind="video"),
}
BUILTIN_TEXT_MODELS = tuple(
    name for name, spec in BUILTIN_MODEL_CATALOG.items() if spec.kind == "text"
)
BUILTIN_IMAGE_MODELS = tuple(
    name for name, spec in BUILTIN_MODEL_CATALOG.items() if spec.kind == "image"
)
BUILTIN_VIDEO_MODELS = tuple(
    name for name, spec in BUILTIN_MODEL_CATALOG.items() if spec.kind == "video"
)
BUILTIN_EXPOSED_MODELS = BUILTIN_TEXT_MODELS + BUILTIN_IMAGE_MODELS + BUILTIN_VIDEO_MODELS
...
class AppConfig:
    ...
    def video_api_base_url(self) -> str:
        return f"{self.glm_base_url}/video-api/v1"

    @property
    def video_chat_url(self) -> str:
        return f"{self.video_api_base_url}/chat"

    @property
    def video_upload_url(self) -> str:
        return f"{self.video_api_base_url}/static/upload"
...
def load_config(env_file: str = ".env") -> AppConfig:
    ...
    config = AppConfig(
        ...
        max_request_body_bytes=parse_int(
            values.get("MAX_REQUEST_BODY_BYTES"), DEFAULT_MAX_REQUEST_BODY_BYTES
        ),
    ...
    if config.max_request_body_bytes <= 0:
        raise ConfigError(f"请求体上限必须大于 0: MAX_REQUEST_BODY_BYTES={config.max_request_body_bytes}")
